...
describe("voteRouter", () => {
  ...
  beforeEach(async () => {
    ...
    await db.insert(datasets).values({ id: "ds-1", name: "Test", ownerAddress: "0xowner", createdAt: new Date() })

    const result = await call(
      contributionRouter.submit,
      { datasetId: "ds-1", contributorAddress: "0xcontributor", shelbyAccount: "shelby://test", data: Buffer.from("test").toString("base64") },
      { context: ctx },
    )
  ...
  it("cast: contribution status'u pending'de bırakır (indexer değiştirir)", async () => {
    ...
    await call(voteRouter.cast, { contributionId, voterAddress: "0xvoter", decision: "approve", aptosTxHash: "0xtx2" }, { context: ctx })
  ...
  it("cast: farklı kararları kaydeder (approve/reject/improve)", async () => {
    await call(voteRouter.cast, { contributionId, voterAddress: "0xv1", decision: "approve", aptosTxHash: "0xt1" }, { context: ctx })
    await call(voteRouter.cast, { contributionId, voterAddress: "0xv2", decision: "reject", aptosTxHash: "0xt2" }, { context: ctx })
    await call(voteRouter.cast, { contributionId, voterAddress: "0xv3", decision: "improve", aptosTxHash: "0xt3" }, { context: ctx })
